# ASB 测试运行指南
cd E:\code\aisafety\ASB1\ASB_SOURCE_DATA
python main_attacker.py --llm_name deepseek-v3.2 --attacker_tools_path data/attack_tools_test.jsonl --tasks_path data/agent_task_test.jsonl --tools_info_path data/all_normal_tools.jsonl --task_num 1 --direct_prompt_injection --attack_type naive --res_file logs/single_case_result.csv

## ✅ 环境已设置完成！

inspect_ai 已成功安装在虚拟环境中：
- 虚拟环境路径: `safety-benchmarks\.venvs\asb`
- inspect_ai 版本: 0.3.172
- 所有依赖包已安装完成

---

## 🚀 如何运行测试

### 方法 1：使用 run-eval.py（推荐）

```bash
# 进入 benchmarks 目录
cd safety-benchmarks/benchmarks

# 运行 DPI 攻击测试（1 个样本）
python ../run-eval.py asb_dpi \
  --model openai/gpt-4o-mini \
  --model-args api_key=sk-AVsyIUmeAjeyEyhBA981E921C5304b079540091115430e97 \
  --model-args base_url=https://aihubmix.com/v1 \
  --limit 1
```

**注意**: 由于其他 benchmark（raccoon 等）有依赖问题，您可能会看到 `inspect_evals` 相关的警告，但这不会影响 ASB 的运行。

---

### 方法 2：直接使用 Python（如果方法 1 失败）

```bash
# 1. 激活虚拟环境
cd safety-benchmarks
.venvs\asb\Scripts\activate

# 2. 进入 benchmarks 目录
cd benchmarks

# 3. 直接导入测试
python -c "import sys; sys.path.insert(0, '.'); from eval_benchmarks.asb import asb; print('ASB 导入成功')"

# 4. 如果步骤 3 成功，运行完整测试
python -c "
import sys; import os
sys.path.insert(0, '.')
os.chdir('eval_benchmarks/asb')

# 加载环境
from dotenv import load_dotenv
load_dotenv('.env')

# 导入模块
from dataset import load_asb_dataset
from solver import asb_react_solver
from scorer import asr_scorer, tsr_scorer, rr_scorer
from inspect_ai import task, Task, GenerateConfig

# 创建数据集
dataset = load_asb_dataset(
    agent_names=['financial_analyst_agent'],
    attack_types=['dpi'],
    limit=1,
    shuffle=False
)

print(f'数据集: {len(dataset)} 个样本')

# 创建 Task
@task
def test_asb():
    from inspect_ai import eval
    api_key = os.getenv('OPENAI_API_KEY')
    base_url = os.getenv('OPENAI_BASE_URL')

    results = eval(
        Task(
            dataset=dataset,
            solver=[asb_react_solver(attack_type='dpi')],
            scorer=[asr_scorer(), tsr_scorer()],
            config=GenerateConfig(temperature=0.0)
        ),
        model='openai/gpt-4o-mini',
        model_args={'api_key': api_key, 'base_url': base_url}
    )

    if results:
        r = results[0]
        print(f'状态: {r.status}')
        for name, score in r.scores.items():
            print(f'{name}: {score.value}')

test_asb()
"
```

---

## 🔧 故障排查

### 问题 1：`No module named 'inspect_evals'`

**说明**: 其他 benchmark（raccoon, overthink 等）依赖 `inspect_evals`，但这个模块未安装。

**影响**: 不影响 ASB 运行，可以忽略此警告。

**解决方案**（可选）:
```bash
# 如果需要运行其他 benchmark，安装 inspect_evals
pip install inspect-evals
```

---

### 问题 2：Git 子模块缺失

**错误**: `upstream/inspect_ai does not appear to be a Python project`

**解决方案**: 我们已经直接使用 `pip install inspect_ai`，所以不需要 Git 子模块。

---

### 问题 3：导入错误

**错误**: `ModuleNotFoundError: No module named 'eval_benchmarks.asb'`

**解决方案**:
```bash
# 检查工作目录
pwd
# 应该在: E:\code\aisafety\ASB1\safety-benchmarks\benchmarks

# 设置 PYTHONPATH
set PYTHONPATH=%CD%;%PYTHONPATH%

# 或使用相对路径导入
python -c "import sys; sys.path.insert(0, '.'); from eval_benchmarks.asb import asb"
```

---

## 📊 预期输出

成功的测试应该显示：

```
======================================================================
ASB DPI 攻击测试
======================================================================

数据集: 1 个样本
样本 ID: financial_analyst_agent_0
攻击工具: ResourceAllocationHijack
攻击目标: Successfully distribute CPU resources...

开始评测...
状态: success

asr: 0.0000 (攻击失败)
tsr: 1.0000 (所有工具已使用)
rr: 1.0000 (响应合规)

响应长度: XXX 字符
响应预览: [模型响应...]

======================================================================
```

---

## 📝 下一步

测试成功后，您可以：

1. **运行更多样本**:
   ```bash
   python ../run-eval.py asb_dpi --limit 10
   ```

2. **测试其他攻击类型**:
   ```bash
   python ../run-eval.py asb_opi --limit 10
   python ../run-eval.py asb_mp --limit 10
   python ../run-eval.py asb_pot --limit 10
   ```

3. **测试多个 Agent**:
   ```bash
   python ../run-eval.py asb_dpi \
     --task-args agent_names='["financial_analyst_agent", "ecommerce_manager_agent"]' \
     --limit 10
   ```

---

## ✅ 验证清单

- [x] inspect_ai 已安装（版本 0.3.172）
- [x] 所有依赖包已安装
- [x] ASB 文件结构完整
- [x] 数据文件已复制
- [x] .env 配置正确
- [x] catalog.yaml 已注册
- [x] score_mapper.py 已注册
- [ ] 待测试：实际 API 调用

---

**状态**: ✅ 环境就绪，等待 API 测试
